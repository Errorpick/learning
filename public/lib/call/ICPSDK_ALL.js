...
/**
 * CloudICP 22.0
 * version: ICP eAPP610_TD V100R022C00SPC100
 */
var sdkVersion = "eAPP610_TD V100R022C00SPC100"
var cloudICP;
            ...
            var checkSDKServerWebSocketIsConnectedFunc = function (callback, data) {
                ...
                var loginWaitTime = setInterval(function () {
                    if (cloudICP.loginFlag) {
                        console.log('SDKServer WebSocket is Connected return.');
                        clearInterval(loginWaitTime);
                        //清理无效视频相关业务的缓存
                        var iResult;
                        Object.keys(cloudICP.util.callStatusMgr.windowInfos).forEach(function (keyWindowInfos) {
                            iResult = false;
                            if (undefined == cloudICP.util.callStatusMgr.windowInfos[keyWindowInfos]["cid"]) {//没有cid
                                delete cloudICP.util.callStatusMgr.windowInfos[keyWindowInfos];
                            } else {
                                if ("" == JSON.stringify(cloudICP.util.callStatusMgr.videoMediaInfoBuf) || "{}" == JSON.stringify(cloudICP.util.callStatusMgr.videoMediaInfoBuf)) {
                                    iResult = true;
                                } else {
                                    Object.keys(cloudICP.util.callStatusMgr.videoMediaInfoBuf).forEach(function (key) {
                                        if (key == cloudICP.util.callStatusMgr.windowInfos[keyWindowInfos]["cid"]) {
                                            iResult = true;
                                            return;
                                        }
                                    });
                        ...
                        count--;
                        console.log('SDKServer WebSocket is not Connected, count=' + count.toString());
                        if (count <= 0) {
                            clearInterval(loginWaitTime);
                            callback(data);
                            return;
...
ICPSDK_Dispatch_Device.prototype.ringType = function (param) {
    if (undefined == param) {
        cloudICP.util.log({ "logLevel": "error", "logMsg": "ringType: param is invalid" });
        return;
    }

    if (undefined == param["callback"] || typeof param["callback"] != "function") {
        cloudICP.util.log({ "logLevel": "error", "logMsg": "ringType: callback is not a function" });
        return;
    }
    ...
    if (!param["ringType"] || typeof (param["ringType"]) != "string" || (param["ringType"] != "0" && param["ringType"] != "1")) {
...
ICPSDK_Dispatch_Device.prototype.setLinkedPhone = function (param) {
    if (undefined == param) {
        cloudICP.util.log({ "logLevel": "error", "setLinkedPhone": "param is invalid" });
        return;
    }

    if (undefined == param["callback"] || typeof param["callback"] != "function") {
        cloudICP.util.log({ "logLevel": "error", "setLinkedPhone": "callback is not a function" });
        return;
    }
            ...
            if (event["param"]["type"] == "4") {
                ...
                setTimeout(function(){
                    ...
                    if (event["param"]["ret"] == "0") {
                        ...
                        cloudICP.dispatch.event.EVENT_LIST["VoiceNotify"]["OnHoldSuccess"](retEvent);
                    } else {
                        var retEvent = {
                ...
                setTimeout(function(){
                    ...
                    if (event["param"]["ret"] == "0") {
                        ...
                        cloudICP.dispatch.event.EVENT_LIST["VoiceNotify"]["OnUnholdSuccess"](retEvent);
                    } else {
                        var retEvent = {
                        ...
                        cloudICP.dispatch.event.EVENT_LIST["VoiceNotify"]["OnUnholdFailure"](retEvent);
                    }
                }, 500);
    ...
    switch(statusType) {
        ...
        case "50":
            if (statusValue == "4092") {
                ...
                this.EVENT_LIST["ResourceNotify"]["ResultOfMappingBetweenIsdn2Gbid"](retEvent);
            }
            break;
        default:
            break;
    ...
    if (caller == cloudICP.userInfo["isdn"]) {
		...
		newEvent["value"]["caller"] = from;
            ...
            case "2058":  //voice linkphone notify
                ...
                this.EVENT_LIST["VoiceNotify"]["OnLinkedPhoneNotify"](newEvent);
        }
    }
};
        ...
        case 196610:
            ...
            if (value["rts_ssrc"] != undefined) {
                if (undefined != event["repeatsMonitor"]) {
                    param = "{\"description\":\"msp_create_video_channel\", \"cmd\": 65557,  \"param\":{\"resID\":{0}, \"winHandle\":{9}, \"mediaInfo\":{\"localIP\":\"{1}\", \"localPort\":{2}, \"remoteIP\":\"{3}\",\"remotePort\":{4}, \"playload\":{5}, \"ssrc\":{6}, \"fmt\":\"{7}\", \"recvOnly\":{8}, \"rtsSSrc\": {10}, \"user\":\"{11}\", \"peerUser\":\"{12}\", \"mode\":\"{13}\", \"port\":{14}, \"fmtType\":\"{15}\", \"width\": {16}, \"height\": {17}, \"videoConfLocalWindow\": {18}  }}}".format(
                ...
                if (undefined != event["repeatsMonitor"]) {
                    param = "{\"description\":\"msp_create_video_channel\", \"cmd\": 65557,  \"param\":{\"resID\":{0}, \"winHandle\":{9}, \"mediaInfo\":{\"localIP\":\"{1}\", \"localPort\":{2}, \"remoteIP\":\"{3}\",\"remotePort\":{4}, \"playload\":{5}, \"ssrc\":{6}, \"fmt\":\"{7}\", \"recvOnly\":{8}, \"user\":\"{10}\", \"peerUser\":\"{11}\", \"mode\":\"{12}\", \"port\":{13}, \"fmtType\":\"{14}\", \"width\": {15}, \"height\": {16}, \"videoConfLocalWindow\": {17}  }}}".format(
...
ICPSDK_Dispatch_Query.prototype.querySpecifiedLevelCamera = function (param) {
    if (undefined == param) {
        cloudICP.util.log({ "logLevel": "error", "querySpecifiedLevelCamera": "param is invalid" });
        return;
    }

    if (undefined == param["callback"] || typeof param["callback"] != "function") {
        cloudICP.util.log({ "logLevel": "error", "querySpecifiedLevelCamera": "callback is not a function" });
        return;
    }
    ...
    if (!(cloudICP.util.isNumber(param["offset"]))) {
        param["offset"] = "0";
        result["desc"] += " querySpecifiedLevelCamera: offset is invalid, use default value offset=0.";
    }

    if (!(cloudICP.util.isNumber(param["limit"]))) {
        param["limit"] = "1000";
        result["desc"] += " querySpecifiedLevelCamera: limit is invalid, use default value limit=1000.";
    } else {
        var iLimit = parseInt(param["limit"]);
        if (1 > iLimit || 5000 < iLimit) {
            param["limit"] = "1000";
            result["desc"] += " querySpecifiedLevelCamera: limit not in [1,5000], use default value limit=1000.";
        }
    }

    var url = cloudICP.getSdkServerUrl() + "/v1/querylist/" + cloudICP.userInfo["isdn"] + "/querylevelcameras?offset={0}&limit={1}".format(
        param["offset"], param["limit"]);
...
ICPSDK_Dispatch_Query.prototype.queryDeviceIDRelationship = function (param) {
    if (undefined == param) {
        cloudICP.util.log({ "logLevel": "error", "queryDeviceIDRelationship": "param is invalid" });
        return;
    }

    if (undefined == param["callback"] || typeof param["callback"] != "function") {
        cloudICP.util.log({ "logLevel": "error", "queryDeviceIDRelationship": "callback is not a function" });
        return;
    }
    ...
    result["rsp"] = "-2";
    if (undefined == param["Type"] || ('0' != param["Type"] && '2' != param["Type"]) ) {
    ...
    if (!(cloudICP.util.isNumber(param["OffSet"])) && '2' == param["Type"]) {
        param["OffSet"] = "0";
        result["desc"] += " queryDeviceIDRelationship: OffSet is invalid, use default value OffSet=0.";
    }

    if (!(cloudICP.util.isNumber(param["Limit"])) && '2' == param["Type"]) {
        param["Limit"] = "10000";
        result["desc"] += " queryDeviceIDRelationship: Limit is invalid, use default value Limit=10000.";
    }

    var url = cloudICP.getSdkServerUrl() + "/v1/querylist/" + cloudICP.userInfo["isdn"] + "/deviceidelationship?offset={0}&limit={1}".format(
        param["OffSet"], param["Limit"]);
...
ICPSDK_Dispatch_Video.prototype.monitorVideo = function(param) {
    ...
    //监控相同摄像头的视频流   不支持用户窗口
    var jsonMediaValue = { "value" : "" };
    if (cloudICP.util.checkUserIsMonitor(cloudICP.userInfo["isdn"], param["to"], jsonMediaValue)) {
        ...
        if (undefined != param["monitorParam"]["userWindowId"]) {
            params = "{\"description\":\"msp_create_video_window\", \"cmd\": 196610, \"param\":{\"resID\":{0}, \"windowName\": \"{1}\", \"mode\": \"{2}\", \"width\": {3}, \"height\": {4}, \"x\": {5}, \"y\":{6}, \"hide\": {7}, \"enableDrag\":{8}, \"scrollX\": {9}, \"scrollY\":{10}, \"showToolbar\": {11}, \"buttonIDs\": {12}, \"userWindowId\": {13}, \"repeatsMonitor\": {14} }}".format(
                monitorRepeatsCid, monitorRepeatsCid, strWindowinfo["mode"], strWindowinfo["width"], strWindowinfo["height"], strWindowinfo["posX"], strWindowinfo["posY"], strWindowinfo["hide"], strWindowinfo["enableDrag"], 0, 0, strWindowinfo["showToolbar"], strWindowinfo["buttonIDs"], param["monitorParam"]["userWindowId"], true
            );
        } else {
            params = "{\"description\":\"msp_create_video_window\", \"cmd\": 196610, \"param\":{\"resID\":{0}, \"windowName\": \"{1}\", \"mode\": \"{2}\", \"width\": {3}, \"height\": {4}, \"x\": {5}, \"y\":{6}, \"hide\": {7}, \"enableDrag\":{8}, \"scrollX\": {9}, \"scrollY\":{10}, \"showToolbar\": {11}, \"buttonIDs\": {12}, \"repeatsMonitor\": {13} }}".format(
                monitorRepeatsCid, monitorRepeatsCid, strWindowinfo["mode"], strWindowinfo["width"], strWindowinfo["height"], strWindowinfo["posX"], strWindowinfo["posY"], strWindowinfo["hide"], strWindowinfo["enableDrag"], 0, 0, strWindowinfo["showToolbar"], strWindowinfo["buttonIDs"], true
            );
        }
        ...
        result["desc"] = "monitorVideo: The object(" + param["to"] + ") is exist repeat monitor, can't monitor again.";
        result["rsp"] = "-5";
        param["callback"](result);
        return;
    }
...
ICPSDK_Dispatch_Video.prototype.release = function(param) {
    ...
    if (undefined != repeats && true == repeats && undefined == cloudICP.util.callStatusMgr.videoMediaInfoBuf[cid]["value"]["repeats"]) {
    ...
    if (undefined != repeats && repeats == true) {
        ...
        if (true == bExistRepeatsMonitor) {//首次监控该对象发起的业务结束，重复监控业务也要被关闭
            var repeatCid = jsonMediaValue["value"]["cid"];
            var params = "{\"description\":\"msp_delete_video_channel\", \"cmd\": 65540, \"param\":{\"resID\":{0}}}".format(
                parseInt(repeatCid)
            );
            cloudICP.dispatch.webSocket.sendDataToMediaSDK(params, true);

            //通知上层OnCallRelease消息
            newEvent["value"]["caller"] = jsonMediaValue["value"]["caller"];
            newEvent["value"]["callee"] = jsonMediaValue["value"]["callee"];
            newEvent["value"]["cid"] = repeatCid;
            newEvent["value"]["fmt"] = jsonMediaValue["value"]["fmt"];
            newEvent["value"]["calltype"] = "monitor";
            newEvent["value"]["releasetype"] = "passive";
            newEvent["value"]["src"] = "";
            newEvent["value"]["direction"] = "out";
            newEvent["eventName"] = "OnCallRelease";
            cloudICP.dispatch.event.EVENT_LIST["VoiceNotify"]["OnCallRelease"](newEvent);

            if (undefined != cloudICP.util.callStatusMgr.videoMediaInfoBuf[repeatCid]) {//该视频窗口有业务正在使用
                delete cloudICP.util.callStatusMgr.videoMediaInfoBuf[repeatCid];
            }
        } else {//只关闭重复监控的业务
            // 删除视频媒体通道
            var params = "{\"description\":\"msp_delete_video_channel\", \"cmd\": 65540, \"param\":{\"resID\":{0}}}".format(
...
ICPSDK_Util.prototype.checkUserIsMonitor = function (caller, callee, jsonMediaValue) {
    var iResult = false;
    Object.keys(cloudICP.util.callStatusMgr.videoMediaInfoBuf).forEach(function (key) {
        if (undefined != cloudICP.util.callStatusMgr.videoMediaInfoBuf[key]["value"]["caller"] && caller == cloudICP.util.callStatusMgr.videoMediaInfoBuf[key]["value"]["caller"]
            && undefined != cloudICP.util.callStatusMgr.videoMediaInfoBuf[key]["value"]["callee"] && callee == cloudICP.util.callStatusMgr.videoMediaInfoBuf[key]["value"]["callee"]) {
            if (undefined != cloudICP.util.callStatusMgr.videoMediaInfoBuf[key]["value"]["repeatsMonitor"]) {
                jsonMediaValue["value"] = JSON.parse(JSON.stringify(cloudICP.util.callStatusMgr.videoMediaInfoBuf[key]["value"])); // 复制对象
                iResult = false;
                return;
            }
            iResult = true;
...
ICPSDK_Util.prototype.CleanThisRepeatsMonitor = function (caller, callee) {
    ...
    if ("" != strCid) {
        delete cloudICP.util.callStatusMgr.videoMediaInfoBuf[strCid];
    }
}
