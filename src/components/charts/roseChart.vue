...
export default {
  ...
  methods: {
    initEcharts() {
      let that = this;
      this.shtChart = that.$echarts.init(that.$refs.NightingaleEcharts);
      this.shtChart.setOption(that.option);
    },
    noDataShow() {
      this.shtChart = this.$echarts.init(this.$refs.NightingaleEcharts);
      this.shtChart.setOption({
        title: {
          text: `暂无数据`,
          show: true,
          textStyle: {
  ...
  mounted() {
    if (!this.TransplantHospitalData.length) {
      this.noDataShow();
      return;
    }
    this.initEcharts();
