  ...
  <div class="box">
    ...
    <div class="rose_legend" v-if="displayModel == 1">
      <div class="legend" v-for="(item, index) in loopData" :key="index">
        <div
          class="legend_color"
          :style="{ backgroundColor: pieceColor[index] }"
        ></div>
        <div
        ...
        <div class="legend_value" :style="{ color: pieceColor[index] }">
          {{ item.num }}
        </div>
...
export default {
  ...
  methods: {
    ...
    convertData(rowData, legends) {
      ...
      returnData.forEach((d) => {
        d.num =
          this.sum == 0 ? '0' : ((d.value / this.sum) * 100).toFixed(3) + '%';
      });
      //进度条模式时 倒序排序
  ...
  mounted() {
    if (this.rowData && this.rowData.length > 0) {
      ...
      dom.innerHTML = '暂无数据';
      dom.style.cssText = `text-align:center; font-size: 30px; color: #fff; border: none;line-height: ${this.Eheight}`;
    }
  },
  destroyed() {
  ...
  watch: {
    rowData: {
      handler(neVal) {
        let that = this;
        if (!neVal.length) {
          let dom = document.querySelector('.wraper');
          dom.innerHTML = '暂无数据';
          dom.style.cssText = `text-align:center; font-size: 30px; color: #fff; border: none;line-height: ${this.Eheight}`;
        }
